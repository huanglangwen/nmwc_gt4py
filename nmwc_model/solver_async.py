...
t0 = get_time()
time = 0.0
blocking = (os.environ['GT4PY_BLOCKING'] == 'True')
record_graph = False
async_context = AsyncContext(num_streams=20, blocking=blocking, sleep_time=0.1, name="test_microphysics")
if (not blocking) and record_graph: async_context.graph_record()
with cupy.cuda.profile():
    for its in range(1, int(nts+1)):
        ...
        async_context.schedule(prog_isendens, sold, snow, snew, unow, dtdx=dtdx, origin=(nb, 0, 0), domain=(nx, 1, nz1))
        async_context.schedule(prog_velocity, uold, unow, unew, mtg, dtdx=dtdx, origin=(nb, 0, 0), domain=(nx1, 1, nz1))
        if irelax == 1:
            async_context.schedule(relax_boundary_noregion, snew, sbnd1, sbnd2, rel_field, rel_mask)
            async_context.schedule(relax_boundary_noregion, unew, ubnd1, ubnd2, rel_field1, rel_mask1)
        ...
        async_context.schedule(diffusion, snew, sold, tau, origin=(nb, 0, 0), domain=(nx, 1, nz))
        async_context.schedule(diffusion, unew, uold, tau, origin=(nb, 0, 0), domain=(nx1, 1, nz))

        async_context.schedule(diag_pressure, snew, prs, prs0, dth=dth, origin=(nb, 0, 0), domain=(nx, 1, nz1))
        async_context.schedule(diag_montgomery, exn, mtg, prs, topo, topofact=topofact, dth=dth, th00=th00, origin=(nb, 0, 0), domain=(nx, 1, nz1))
        async_context.schedule(diag_height, zhtnow, exn, prs, th0, topo, topofact=topofact, origin=(nb, 0, 0), domain=(nx, 1, nz1))
        ...
        if np.mod(its, iout) == 0:
            ...
            if iprtcfl == 1:
                ...
                print("============================================================\n")
    async_context.wait()
if (not blocking) and record_graph: async_context.graph_save()
t1 = get_time()
print(f"Blocking: {blocking}, Backend: {BACKEND}, Elapsed time: {t1-t0} s")
